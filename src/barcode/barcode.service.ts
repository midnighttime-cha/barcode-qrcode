...
export class BarcodeService {
  async genBarcode(query: any = null) {
    try {
      if (query) {
        const { format, code, text, background, lineColor, textMargin, textAlign, textPosition, margin, marginBottom, marginTop, fontOptions, font, fontSize } = query;
        let options = {};
        ...
        const canvasDOM = createCanvas(200, 200);

        JsBarcode(canvasDOM, `${code}`, { ...options, flat: true });
        Logger.log(canvasDOM, "canvasDOM");
